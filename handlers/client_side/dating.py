...
from databases.admin_side import ComplainsDB
            ...
            if check_register_user:
                ...
                while 1:
                    if users_id and len(users_id) != 1:
                        ...
                        await message.answer('На данный момент нет ни одного пользователя, который подходит Вам!')
                        break
            else:
...
async def score_and_next_review(callback: CallbackQuery):
    ...
    try:
        ...
        if check_register_user:
            user_sex, interesting_sex = await get_attributes_user_for_dating(register_db, user_id=callback.from_user.id)
            users_id = register_db.get_users_id_by_interesting_sex(user_sex=user_sex, interesting_sex=interesting_sex)
...
async def complain_handler(callback: CallbackQuery):
    ...
    await callback.answer('Жалоба успешно отправлена')
...
async def write_to_the_user(callback: CallbackQuery):
    ...
    if vip_db.exists_user(user_id=callback.from_user.id):
        ...
        age, city, name_user, description_user, photo, instagram = await \
            get_attributes_user_for_profile(register_db=register_db, user_id=user_id)
        await callback.message.answer(text=f'Надеюсь ты хорошо проведешь время!\n\n'
                                           f'👉<a href="tg://user?id={user_id}">{name_user}</a>', parse_mode='html')
        await callback.message.answer('Если по каким-либо причинам ссылка неактивна, то вероятнее всего аккаунт '
                                      'был удален или же по неизвестным причинам доступ был к нему ограничен')
    else:
        await callback.message.answer('У Вас нет статуса VIP! Данная функция доступна только при получении '
                                      'статуса VIP\n\n'
                                      '"/vip" - приобрести VIP👑')


def register_dating_handlers():
    dp.register_message_handler(questionnaire_review, Text(equals=['Смотреть анкеты🚀']))
